...
def send_daily_reminders():
    ...
    users_to_remind = User.query.filter(~User.reservations.any(Reservation.parking_timestamp > yesterday)).all()
...
def export_reservations_as_csv(user_id):
    ...
    writer.writerow(['Reservation ID', 'Spot ID', 'Parking Lot', 'Parking Timestamp', 'Leaving Timestamp', 'Cost'])

    for res in reservations:
        parking_lot_name = ""
        if res.spot:
            parking_lot_name = res.spot.parking_lot.name
        writer.writerow([res.id, res.spot_id, parking_lot_name, res.parking_timestamp, res.leaving_timestamp, res.parking_cost])
